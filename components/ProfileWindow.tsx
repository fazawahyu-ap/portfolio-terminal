...
export default function ProfileWindow({ onClose, onMinimize, isMinimized, isExpanded }: ProfileWindowProps) {
  ...
  return (
    <div className={`w-full transition-all duration-500 ease-in-out overflow-hidden flex flex-col bg-[#1e1e1e]/95 backdrop-blur-xl border border-emerald-500/20 shadow-[0_0_30px_rgba(16,185,129,0.1)] rounded-xl group hover:shadow-[0_0_40px_rgba(16,185,129,0.2)] ${
      isMinimized ? "h-10 min-h-[40px]" : "h-[520px] max-h-[75vh]"
    }`}>
      
      {/* Title bar (Area ini BISA diseret) */}
      <div className="flex items-center h-10 px-4 bg-[#2b2b2b] border-b border-black/40 shrink-0 select-none cursor-grab active:cursor-grabbing">
        ...
        {/* Tombol Aksi (Area ini menahan seretan agar klik bekerja) */}
        <div className="flex items-center gap-2" onPointerDown={(e) => e.stopPropagation()}>
          <button onClick={onClose} className="w-3 h-3 rounded-full bg-[#ff5f57] border border-black/10 hover:brightness-110 active:brightness-90"></button>
          <button onClick={onMinimize} className="w-3 h-3 rounded-full bg-[#febc2e] border border-black/10 hover:brightness-110 active:brightness-90"></button>
      ...
      {/* Konten (Area ini BISA di-scroll/diblok, TIDAK BISA diseret) */}
      <div 
